...
function findPositionInOrder(lid, ltype, orderList){
    return orderList.reduce(function(a, b){
        console.log('ordenando...', b.layer_type, b.id_layer, lid, ltype, orderList, b.position);
...
$('.input-group').change(function(e){
    if($(this).is(':checked')){
        $(this).closest('li').find('.collapsible-body').find('li').each(function(idx, el){
...
$('.input-user').change(function(e){
    ...
    $($(this).parents('li').get(0)).find('.collapsible-header').find('input').prop('checked', allChecked);
